#Ahora intentamos conectar por rtsp coordinando el time
...
INFRA_COOLDOWN_MS = 1500       # 1.5s típico

# Umbral de prueba (3 km/h, cambiarlo en producción)
SPEED_LIMIT_KMH = 20
...
VEHICLE_LABELS = {"car", "bus", "truck", "motorbike", "bicycle", "motorcycle"}


def get_signal_state_safe(cfg, semaforo_state, signal_id=None):
    """
    Devuelve un dict normalizado: {"state": "RED|GREEN|YELLOW|GRAY", "age": segundos (float)}
    - Si no hay dato o está 'stale' => retorna GRAY y age=None.
    """
    try:
        stale_after = float(cfg.get("stale_after_sec", 5.0))
    except Exception:
        stale_after = 5.0

    # 1) Elige ID principal
    sid = (signal_id or cfg.get("primary_signal_id") or None)
    snap = None
    now = time.time()

    # 2) Obtén snapshot (robusto)
    if sid and hasattr(semaforo_state, "get_view"):
        snap = semaforo_state.get_view(str(sid).upper())
    elif hasattr(semaforo_state, "get_view_all"):
        allv = semaforo_state.get_view_all()
        snap = allv[0] if allv else None

    # 3) Normaliza
    if not isinstance(snap, dict):
        return {"state": "GRAY", "age": None}

    st = (snap.get("state") or "GRAY").upper()
    t  = snap.get("t", None)
    age = (now - float(t)) if isinstance(t, (int, float)) else None

    # 4) Si está 'stale' o estado desconocido => GRAY
    if st not in ("RED","GREEN","YELLOW"):
        return {"state":"GRAY", "age":None}
    if age is not None and age > stale_after:
        return {"state":"GRAY", "age":None}

    return {"state": st, "age": age}

def draw_signal_hud(frame, cfg, semaforo_state, where=(20, 20)):
    """
    Dibuja en pantalla un mini panel con el estado actual de cada semáforo:
    - Círculo RED/GREEN/YELLOW/GRAY
    - Texto con ID y edad (s)
    """
    import cv2
    x0, y0 = where
    pad = 8
    ...
    for s in signals:
        sid = s.get("id", "S?")
        ss = get_signal_state_safe(cfg, semaforo_state, sid)
        col = {"RED":(0,0,255), "GREEN":(0,255,0), "YELLOW":(0,255,255), "GRAY":(180,180,180)}.get(ss["state"], (180,180,180))

        # círculo
        cv2.circle(frame, (x0+radius, y+radius), radius, col, -1, lineType=cv2.LINE_AA)
        # contorno suave
        cv2.circle(frame, (x0+radius, y+radius), radius, (40,40,40), 2, lineType=cv2.LINE_AA)

        # texto
        age_txt = "" if ss["age"] is None else f"{ss['age']:.1f}s"
        txt = f"{sid}: {ss['state']}" + (f"  {age_txt}" if age_txt else "  (no data)")
        cv2.putText(frame, txt, (x0 + 2*radius + pad, y+radius+5), font, 0.5, (255,255,255), 2, cv2.LINE_AA)
        cv2.putText(frame, txt, (x0 + 2*radius + pad, y+radius+5), font, 0.5, (0,0,0), 1, cv2.LINE_AA)
...
def detection_and_tracking(raw_queue, processed_queue, stop_event):
    ...
    while not stop_event.is_set() or not raw_queue.empty():
        ...
        for track in tracks:
            ...
            if (label in VEHICLE_LABELS) and STOP_LINE and len(STOP_LINE) == 2:
                A, B = STOP_LINE
            ...
            # --- CROSSWALK (bloqueo por detención en paso peatonal) SOLO VEHÍCULOS ---
            if label in VEHICLE_LABELS:
                ...
                # si el track no es vehículo, limpia el contador para ese ID
                on_crosswalk_frames.pop(track_id, None)
            ...
            # ===== PEATÓN: cruce prohibido por NO ser su luz (semáforo vehicular no en ROJO) =====
            # Solo aplica si es persona y está pisando calzada (ROI) o el paso peatonal.
            if label == "person":
                in_paso_now = any(point_in_polygon((cx, cy), poly) for poly in CROSSWALKS_POLYS) if CROSSWALKS_POLYS else False
                in_road_now = any(point_in_polygon((cx, cy), r["poly"]) for r in rois)

                if in_road_now or in_paso_now:
                    snap = get_signal_state_safe(cfg, semaforo_state)  # normaliza a RED/GREEN/YELLOW/GRAY y filtra 'stale'
                    # No sancionamos si es GRAY (desconectado/sin dato). Solo GREEN o YELLOW.
                    if snap["state"] in ("GREEN", "YELLOW"):
                        note = "Peatón circula sin rojo vehicular (semáforo no está en ROJO)"
                        emit_infraction_once(
                            track_id, "PEDESTRIAN_CROSS_FORBIDDEN",
                            pos_msec, timestamp_str,
                            roi_for_event, frame, (x1, y1, x2, y2),
                            valor="", umbral="", nota=note
                        )
                        cv2.putText(frame, "PEDESTRIAN_CROSS_FORBIDDEN",
                                    (x1, max(20, y1-65)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
            # ===== /PEATÓN: cruce prohibido por NO ser su luz =====
        ...
        # === HUD del semáforo (estado/edad por pantalla) ===
        draw_signal_hud(frame, cfg, semaforo_state, where=(20, 20))
