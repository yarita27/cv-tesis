...
# === VIDEO & DETECCIÓN ===
VIDEO_SOURCE = r"videos/velocidad.mp4" 
    ...
    while not stop_event.is_set() or not raw_queue.empty():
        ...
        for track in tracks:
            ...
            if prev_state.startswith("ROI") and actual_roi.startswith("ROI") and prev_state != actual_roi:
                # 🚨 Detectó cambio de carril (ROI distinto)
                # Solo aplicar si estamos en zona prohibida (ej. flag en el JSON de geometría)
                if geo.get("forbidden_lane_change", True):  # bandera activada en la config
                    ...
                    if emit_infraction_once(track_id, "LANE_CHANGE_FORBIDDEN", pos_msec, timestamp_str,
                                            ...
                                            valor="", umbral="", nota=note):
                        cv2.putText(frame, "LANE_CHANGE_FORBIDDEN", (x1, y1-50),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
